...
Find Reach events with Cylindoor
"""
import numpy as np
import pandas as pd
import os
...
def find_reach_events(session, vid_tag):
    ...
    #                 pDetect.append(frame_number)
    #             except ValueError:
    #                 print(f"Warning: Skipped Line '{line.strip()}' Because No Frame Present.")
    # print(f'frame_list detected:{frame_list}')
        ...
        for line in file:
            if "pellet_delivery" in line:
                try:
                    ...
                    print(f"Warning: Skipped Line '{line.strip()}' Because No Frame Present.")
    print(f'frame_list delivery:{frame_list}')
    if not frame_list:
        print("No 'pellet_delivery' events found in the file.")
    ...
    start_height = -3 #-4
    start_distance = 10
    reach_init_speed = -0.025 #-0.025
    reach_dirchange_speed = 0.025
    pellet_drop_speed = 0.65 #0.225
    pellet_drop_dist = -5
    pDrop_xDist = 5
    pellet_punch_dist = -3
    dist_thresh_end = 5 #6
    ...
    for frmindex, start_frame in enumerate(frame_list):
        ...
        sfpx = None
        missed = False
        start_height = -3 #redefine to account for previous miss threshold changes
        start_distance = 10
        #define dist and velo for each reach sequence (changes according to start_frame)
        if frmindex < len(frame_list)-1:
            search_end = frame_list[frmindex+1]-late_reach_buff
            print(f'search_end:{search_end}')
        else:
            search_end = frm_count-late_reach_buff
        pellet_was_detected = False
        search_list = np.arange(start_frame-150,search_end)
        for sfp in search_list.tolist():
            ...
            if testB:# and testA:
                ...
                for xframe in range(sfp, search_end - batch_frm):
                   ...
                   if np.all(filt_data['Pellet']['x_likelihood'][xframe:xframe + batch_frm] > confidence):
                       # Check if the position stays relatively consistent within 1mm in either direction
                       if (np.max(filt_data['Pellet']['x_filt'][xframe:xframe + batch_frm]) - np.min(filt_data['Pellet']['x_filt'][xframe:xframe + batch_frm]) < 2.0):
                           sfpx = xframe
                           break
                if sfpx == None:
                    sfpx = sfpyz
                # print(f'sfpyz = {sfpyz}, sfpx = {sfpx}')
                distance_p = np.sqrt((filt_data['Pellet']['x_filt'].values-filt_data['Pellet']['x_filt'].values[sfpx])**2 + (filt_data['Pellet']['y_filt'].values-filt_data['Pellet']['y_filt'].values[sfpyz])**2 + (filt_data['Pellet']['z_filt'].values-filt_data['Pellet']['z_filt'].values[sfpyz])**2)
                distance_hvpp = np.sqrt((filt_data['Hand']['x_filt'].values-filt_data['Pellet']['x_filt'].values[sfpx])**2 + (filt_data['Hand']['y_filt'].values-filt_data['Pellet']['y_filt'].values[sfpyz])**2 + (filt_data['Hand']['z_filt'].values-filt_data['Pellet']['z_filt'].values[sfpyz])**2)
                Z_dist_h = filt_data['Pellet']['z_filt'].values[sfpyz] - filt_data['Hand']['z_filt'].values
                Z_dist_p = filt_data['Pellet']['z_filt'].values[sfpyz]-filt_data['Pellet']['z_filt'].values
                Y_dist_p = filt_data['Pellet']['y_filt'].values[sfpyz]-filt_data['Pellet']['y_filt'].values
                X_dist_p = filt_data['Pellet']['x_filt'].values[sfpx]-filt_data['Pellet']['x_filt'].values
        ...
        while frame < frm_count - batch_frm:             # test if we reached the next pellet placement 
            if frmindex < len(frame_list)-1 and frame >= frame_list[frmindex+1]-late_reach_buff: 
                if search_status == 1:
                    if debug:
            ...
            if search_status == 1:                  # search for a reach initiation
                ...
                if pellet_detected == True:
                    ...
                    if testD and testE:                        
                        testA = Z_dist_h[frame] < start_height 
                        testB = np.mean(velocity_h_filt[frame:frame+batch_speed]) < reach_init_speed
                        testC = distance_hvpp[frame] > start_distance
                        if testB and (testA or testC): 
                            if debug:
                ...
                if np.any(filt_data['Pellet']['speed_filt'][frame:frame + batch_drop] > pellet_drop_speed):
                    ...
                    # print('Pellet dropped via Speed MAX')
                if np.any(Y_dist_p[frame:frame+batch_drop] < pellet_punch_dist):
                    food_was_dropped = True 
                    # print('Pellet dropped via ydist MAX')
                if np.any(Z_dist_p[frame:frame+batch_drop] < pellet_drop_dist): #food dropped if pellet is too low - 
                    ...
                    if testD:
                        food_was_dropped = True   
                        # print('Pellet dropped via zdist MAX')
                # if np.any(X_dist_p[frame:frame+batch_drop] < -pDrop_xDist) or np.any(X_dist_p[frame:frame+batch_drop] > pDrop_xDist-1.5):
                #     # if np.any(np.diff(X_dist_p[frame:frame+batch_drop])>1)==False and np.any(np.diff(X_dist_p[frame:frame+batch_drop])<-1)==False:
                #     if np.any(X_dist_p[frame:frame+batch_drop] > 5) == False:
                #         food_was_dropped = True 
                if frame<sfpx: #test to limit effect of bad tracking on pellet origin
                    food_was_dropped = False
                # testB = np.mean(speed_hvh_init[frame:frame+batch_speed]) > reach_dirchange_speed
                testB = np.isclose(np.mean(speed_hvh_init[frame:frame+batch_speed]), reach_dirchange_speed, atol = 0.035) #FLAG
                if testB:
                    if debug:
                        print('reach max at frame %d!' % int(frame+1))
                    reach_events.append(('reachMax', int(frame+1)))
                    max_frm = frame
                ...
                testA = np.mean(distance_hvpp[frame:frame + batch_speed]) > dist_thresh_end  # test if hand is far enough away and moving away from pellet                             
                testB = np.mean(speed_hvh_init[frame:frame+batch_speed]) > 0#reach_dirchange_speed
                testC = np.mean(velocity_h_filt[frame:frame+batch_speed]) < 0#reach_init_speed
                testD = np.isclose(np.mean(distance_hvpp[frame:frame+batch_stall]), distance_hvpp[frame], atol = 2)
                testE = np.isclose(np.mean(filt_data['Hand']['speed_filt'][frame:frame + batch_stall]), 0, atol = 0.025) 
                testF = np.all(distance_hvpp[frame:frame + batch_stall] < 5) #4
                testG = np.any(X_dist_p[frame-10:frame+batch_drop] > 5) == False

                if np.any(filt_data['Pellet']['speed_filt'][frame:frame + batch_drop] > pellet_drop_speed):
                    food_was_dropped = True 
                    # print('Pellet dropped via Speed')
                if np.any(Y_dist_p[frame:frame+batch_drop] < -3):
                    food_was_dropped = True 
                    # print('Pellet dropped via yDist')
                if np.any(Z_dist_p[frame:frame+batch_drop] < pellet_drop_dist): #food dropped if pellet is too low
                    z_dist_indices = np.where(Z_dist_p[frame:frame+batch_drop] < pellet_drop_dist)
                    testD = np.any(filt_data['Pellet']['yz_likelihood'][frame:frame+batch_drop].iloc[z_dist_indices] > confidence)
                    if testD:
                        food_was_dropped = True
                        # print('Pellet dropped via zdist')
                if np.any(X_dist_p[max_frm:frame+batch_drop] < -pDrop_xDist) or np.any(X_dist_p[max_frm:frame+batch_drop] > pDrop_xDist):
                    if np.any(X_dist_p[max_frm:frame+batch_drop] > 5.25) == False:
                        # print('dropped via x pellet pos!!')
                        food_was_dropped = True 

                if testA and testC and not food_was_dropped: #and pellet_detected: #and testC
                    if debug:
                ...
                #     testX = end_query - start_query < 12
                #     testY = end_query - start_query > 150
...
# vid_tag = '_264.mp4'
# sesslist = []
# # sesslist.append(r'Z:\PHYS\ChristieLab\Data\MSP_Z\Reach_Training\20231222\christie2P\session002')
# # sesslist.append(r'Z:\PHYS\ChristieLab\Data\MSP_Z\Reach_Training\20240116\christie2P\session001')
# sesslist.append(r'Z:\PHYS\ChristieLab\Data\MSP_Z\Reach_Training\20240130\christie2P\session002')
# # sesslist.append(r'Z:\PHYS\ChristieLab\Data\MSP_Z\Reach_Training\20240213\christie2P\session004')
