            ...
            for ndx, s in enumerate(dirlist):
                ...
                if len(vid_list):
                    ...
                    for v in vid_list:
                        ...
                        if passval:
                            os.remove(v)
                            print('Successfully transferred %s' % vid_name.stem)
                        else:
                            print('Error transferring %s' % vid_name.stem)
...
def create_training_dataset_CLARA(config,num_shuffles=1,Shuffles=None,windows2linux=False,trainIndexes=None,testIndexes=None):
    ...
    for i in video_names:
        try:
            ...
            try:
                data = pd.read_hdf((str(data_path / Path(i))+'/CollectedData_'+'christie.h5'),'df_with_missing')
                smlData = data.dropna(how='all')
                smlKeys = list(smlData.index.values)
                smlKeyLong = list()
                for sk in smlKeys:
                ...
                smlData.index = smlKeyLong
                data = smlData
                data.columns = data.columns.set_levels(['WRW'], level="scorer")
                if AnnotationData is None:
                    AnnotationData=data
                else:
                    AnnotationData=pd.concat([AnnotationData, data])
    ...
    net_type = str(cfg['default_net_type'])
    import deeplabcut
...
def analyze_videos_CLARA(config_path, v, crp):
    import deeplabcut
    deeplabcut.analyze_videos(config_path,v, videotype='.mp4',
...
def read_dlc_config(configname):
    """
